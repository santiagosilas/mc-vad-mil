...
def train_mcmil1(list_data_paths, iterations = 100, batch_size = 60, feature_dim = 1024):
  """
  @summary: Função para treinar uma rede neural para detecção de anomalias em vídeos de
  vigilância com múltiplas câmeras sobrepostas e com rótulos de vídeo. 
  @param list_paths: Lista de Caminhos para os Dados de Treino de cada câmera. Cada vetor de características possui 1024 atributos, referente às features I3D-RGB.
  @param iterations: Número de iterações de treinamento.
  @param batch_size: Tamanho do batch, onde devem existir batch_size/2 bags normais e batch_size/2 bags anômalas. 
  ...
  model = RegressionNetwork(feature_dim).to(device).train()
  optimizer = torch.optim.Adadelta(model.parameters(), lr=args.lr_base, eps=1e-8)

  criterions = [ RegularizedLoss(model).to(device) for i in range(len(list_data_paths))]
  ...
  for iteration, (data_cams) in enumerate(zip(*train_loaders)):
      ...
      for i in range(len(train_loaders)):  
        ...
        outputs_ci = [model(b) for b in batch_ci]
        loss_ci = criterions[i](outputs_ci, targets_ci)
        loss_cams.append(loss_ci)

      optimizer.zero_grad()

      loss_final = loss_cams[0]
      for i in range(len(train_loaders)):
        loss_final = torch.max(loss_final, loss_cams[i])
...
def evaluation(model, Views, path_cams):
    ...
    for bags_cams in zip(*[dct_test_data[feat][view] for view in Views]):
        ...
        for i in range(len(bags_cams)):
            ...
            out_cam_i = model(features_cam_i.float()).squeeze(-1)
...
if __name__ == "__main__":
    ...
    trained_model = train_mcmil1(
        list_data_paths = [
            "/media/dev/LaCie/MC-VAD-MIL-OUT/pets2009-sultani-format-i3d/pets2009-rgb-seed-1-train-view-001-ekos-input",
    ...
    auc_data, df = evaluation(
        ...
        Views=["001","002","003"],
        path_cams = {
            "001":"/media/dev/LaCie/MC-VAD-MIL-DATA/center-crop/pets2009-relabeled-i3d/pets2009-rgb-seed-1-test-view-001.npy",
