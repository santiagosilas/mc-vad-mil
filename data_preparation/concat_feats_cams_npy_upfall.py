...
if __name__ == "__main__":
    ...
    npy_dir = f"/media/dev/LaCie/MC-VAD-MIL-DATA/center-crop/upfall-i3d"
    out_dir = f"/media/dev/LaCie/MC-VAD-MIL-OUT/upfall-i3d-featconc-{overlapping_cams}cams"

    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    get_filename = lambda fmode, split, cam:     f"upfall-{fmode}-{split}-single-camera-{cam}-seed1.npy"
    get_out_filename = lambda fmode,split,views: f"upfall-{fmode}-{split}-single-camera-views-{views}.npy"
