...
for count_cams in [2,3,4]:
    for views in itertools.combinations(["001","002","003","004"],count_cams):
        key=str(views)\
            .replace("'" , "")\
            .replace(" " , "")\
            .replace("(" , "")\
        ...
        train_path = os.path.join(
            f'/media/dev/LaCie/MC-VAD-MIL-OUT/pets2009-relabeled-i3d-featconc-{len(views)}cams',
            f'pets2009-{mode}-featconc-train-views-{key}.npy')
        
        test_path = os.path.join(
            f'/media/dev/LaCie/MC-VAD-MIL-OUT/pets2009-relabeled-i3d-featconc-{len(views)}cams',
            f'pets2009-{mode}-featconc-test-views-{key}.npy')
        
        output = f'/media/dev/LaCie/MC-VAD-MIL-OUT/pets2009-relabeled-i3d-featconc-milbkbn-wan'
