...
if __name__ == "__main__":
    ...
    npy_dir = "/media/dev/LaCie/MC-VAD-MIL-DATA/center-crop/pets2009-relabeled-i3d"
    out_dir = f"/media/dev/LaCie/MC-VAD-MIL-OUT/pets2009-relabeled-i3d-featconc-{overlapping_cams}cams"

    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    get_filename = lambda fmode, split, cam: f"pets2009-{fmode}-seed-1-{split}-view-{cam}.npy"
    get_out_filename = lambda fmode,split,views: f"pets2009-{fmode}-featconc-{split}-views-{views}.npy"



    camera_views = ["001","002", "003", "004"]
