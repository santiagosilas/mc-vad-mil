...
if __name__ == '__main__':
    ...
    if args.out_foldername == None or args.out_foldername == "None":
        mytime = datetime.datetime.now()
        mynamestr = "{}-{:02d}-{:02d}-{:02d}-{:02d}-{:02d}".format(mytime.year, mytime.month, mytime.day, mytime.hour, mytime.minute, mytime.second)
    else:
        mynamestr = args.out_foldername


    save_path = os.path.join(args.model_name, args.feature_pretrain_model, f"{args.dataset_name_camA}-{args.dataset_name_camB}", 
                             'k_{}'.format(args.k), '_Lambda_{}'.format(args.Lambda), 
                             args.feature_modal, 
                             '{}-SS{}-LC{}-LF{}{}'.format(mynamestr, 
                                                                                 args.sample_size,
    ...
    ckpt_path = os.path.join(os.path.join(os.path.join(args.dataset_path, f"outputs-{args.dataset_name_camA}-{args.dataset_name_camB}"), "ckpt"), save_path)
    if not os.path.exists(ckpt_path):
        os.makedirs(ckpt_path)
    
    logs_path = os.path.join(os.path.join(os.path.join(args.dataset_path, f"outputs-{args.dataset_name_camA}-{args.dataset_name_camB}"), "logs"), save_path)
    if not os.path.exists(logs_path):
        os.makedirs(logs_path)
    logger = Logger(logs_path)

    results_path = os.path.join(os.path.join(os.path.join(args.dataset_path, f"outputs-{args.dataset_name_camA}-{args.dataset_name_camB}"), "result"), save_path)
