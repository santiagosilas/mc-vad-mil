    ...
    if zip:
        ...
        for k, v in predict_dict.items():
            ...
            if os.path.exists(os.path.join(save_root, save_path, 'plot', 'itr_{}'.format(itr))) == 0:
                ...
                plt_path = os.path.join(save_root, save_path, 'plot', 'itr_{}'.format(itr), k)
                print("plt_path", plt_path)
                plt.savefig(plt_path)
            else:
                plt_path = os.path.join(save_root, save_path, 'plot', 'itr_{}'.format(itr), k).replace(".","")
                print("plt_path", plt_path)
                plt.savefig(plt_path)
            plt.close()
