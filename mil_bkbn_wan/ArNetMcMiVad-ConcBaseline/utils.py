...
def anomap(predict_dict, label_dict, save_path, epoch, itr, save_root, zip=False):
    """
    ...
    if zip:
        zip_file_name = os.path.join(save_root, save_path, 'plot', 'epoch_{}_itr_{}.zip'.format(epoch, itr))   
        ...
        for k, v in predict_dict.items():
            predict_np = v.repeat(16)
            label_np = label_dict.item().get(k)[:len(v.repeat(16))]
            x = np.arange(len(predict_np))
            ...
            plt.legend()
            # plt.show()
            if os.path.exists(os.path.join(save_root, save_path, 'plot', 'epoch_{}_itr_{}'.format(epoch,itr))) == 0:
                os.makedirs(os.path.join(save_root, save_path, 'plot', 'epoch_{}_itr_{}'.format(epoch,itr)))
                plt.savefig(os.path.join(save_root, save_path, 'plot', 'epoch_{}_itr_{}'.format(epoch,itr), k))
            else:
                plt_path = os.path.join(save_root, save_path, 'plot', 'epoch_{}_itr_{}'.format(epoch,itr), k).replace(".","")
