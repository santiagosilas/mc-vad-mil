...
def eval_p(epoch, itr, dataset_camA,dataset_camB, dataset_camC, dataset_camD, predict_dict, logger, save_path, args, plot=False, zip=False, manual=False):
    global label_dict_path
    if manual:
        save_root = './manul_test_result'
    else:
        save_root = os.path.join(os.path.join(args.dataset_path, f"outputs-{args.dataset_name_camA}-{args.dataset_name_camB}-{args.dataset_name_camC}-{args.dataset_name_camD}"), "result")
    ...
    all_predict_np_camA, all_predict_np_camB,all_predict_np_camC,all_predict_np_camD,  all_label_np = np.zeros(0), np.zeros(0), np.zeros(0), np.zeros(0), np.zeros(0)
    ...
    if args.late_fusion == "Max":
        all_predict_np = np.maximum(all_predict_np_camA, np.maximum(all_predict_np_camB, np.maximum(all_predict_np_camC, all_predict_np_camD)))
