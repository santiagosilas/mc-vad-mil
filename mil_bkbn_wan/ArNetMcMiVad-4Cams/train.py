...
def train(epochs, train_loader, test_loader, args, model, optimizer, logger, device, save_path, 
    ...
    for epoch in range(1, epochs+1):
        ...
        for i, data in enumerate(train_loader):
            itr += 1
            [anomaly_features_camA, normaly_features_camA],[anomaly_features_camB, normaly_features_camB],[anomaly_features_camC, normaly_features_camC],[anomaly_features_camD, normaly_features_camD], [anomaly_label, normaly_label], stastics_data = data
            ...
            if args.loss_combination == "PairwiseMeanOfClipScoresInBags":
                ...
                element_logits = (element_logits_camA+element_logits_camB+element_logits_camC+element_logits_camD)/4
            ...
            if args.loss_combination == "Max":
                total_loss = torch.max(total_loss_camA, torch.max(total_loss_camB, torch.max(total_loss_camC, total_loss_camD)))
        ...
        if epoch % args.save_every == 0:
            ...
            results = eval_p(epoch = epoch, itr=itr, dataset_camA = args.dataset_name_camA, dataset_camB = args.dataset_name_camB, dataset_camC = args.dataset_name_camC, dataset_camD = args.dataset_name_camD, 
