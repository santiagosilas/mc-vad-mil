...
class dataset(Dataset):
    def __init__(self, args, train=True, trainlist=None, testlist=None):
        ...
        self.dataset_name_camE = args.dataset_name_camE
        ...
        if self.feature_pretrain_model == 'c3d' or self.feature_pretrain_model == 'c3d_ucf':
            ...
            self.feature_path_camD = os.path.join(self.dataset_path, self.dataset_name_camD, 'features_video',
                                             self.feature_pretrain_model, self.feature_layer, self.feature_modal)
            self.feature_path_camE = os.path.join(self.dataset_path, self.dataset_name_camE, 'features_video',
                                             self.feature_pretrain_model, self.feature_layer, self.feature_modal)
        ...
        self.videoname_camE = os.listdir(self.feature_path_camE)
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            ...
            anomaly_start_index_camE = []
            anomaly_len_index_camE = []
            ...
            normaly_start_index_camE = []
            normaly_len_index_camE = []
            ...
            anomaly_features_camE = torch.zeros(0)
            normaly_features_camE = torch.zeros(0)
            ...
            for a_i, n_i in zip(anomaly_indexs, normaly_indexs):
                ...
                # cam E
                anomaly_feature = np.load(file=os.path.join(self.feature_path_camE, anomaly_data_video_name, 'feature.npy'))
                anomaly_len_index_camE.append(anomaly_feature.shape[0])
                anomaly_feature, r = utils.process_feat_sample(anomaly_feature, self.t_max)
                anomaly_start_index_camE.append(r)
                anomaly_feature = torch.from_numpy(anomaly_feature).unsqueeze(0)
                normaly_feature = np.load(
                    file=os.path.join(self.feature_path_camE, normaly_data_video_name, 'feature.npy'))
                normaly_len_index_camE.append(normaly_feature.shape[0])
                normaly_feature, r = utils.process_feat(normaly_feature, self.t_max, self.args.sample_step)
                normaly_feature = torch.from_numpy(normaly_feature).unsqueeze(0)
                normaly_start_index_camE.append(r)
                anomaly_features_camE = torch.cat((anomaly_features_camE, anomaly_feature),
                                                dim=0)  # combine anomaly_feature of different a_i
                normaly_features_camE = torch.cat((normaly_features_camE, normaly_feature),
                                                dim=0)  # combine normaly_feature of different n_i
            ...
            start_index_camE = anomaly_start_index_camE + normaly_start_index_camE
            len_index_camE = anomaly_len_index_camE + normaly_len_index_camE

            return [anomaly_features_camA, normaly_features_camA],[anomaly_features_camB, normaly_features_camB],[anomaly_features_camC, normaly_features_camC],[anomaly_features_camD, normaly_features_camD],[anomaly_features_camE, normaly_features_camE], [anomaly_label, normaly_label], [train_video_name, start_index_camA, len_index_camA]
        else:
            ...
            self.feature_camE = np.load(file=os.path.join(self.feature_path_camE, data_video_name, 'feature.npy'))
            return self.feature_camA,self.feature_camB,self.feature_camC,self.feature_camD,self.feature_camE, data_video_name
