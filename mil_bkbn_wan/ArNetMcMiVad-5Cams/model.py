...
class Model_single_Mtv(torch.nn.Module):
    """
    @summary: multiview network - Date 22 Nov 2023
    """
    def __init__(self, n_feature):
        super(Model_single_Mtv, self).__init__()

        self.fc0_camA = nn.Linear(n_feature, n_feature)
        self.fc0_camB = nn.Linear(n_feature, n_feature)
        self.fc0_camC = nn.Linear(n_feature, n_feature)
        self.fc0_camD = nn.Linear(n_feature, n_feature)
        self.fc0_camE = nn.Linear(n_feature, n_feature)
    ...
    def forward_camA(self, inputs, is_training=True):
        x = F.relu(self.fc0_camA(inputs))
        x = F.relu(self.fc(x))
        if is_training:
            x = self.dropout(x)
        return x, self.sigmoid(self.classifier(x))
    
    def forward_camB(self, inputs, is_training=True):
        x = F.relu(self.fc0_camB(inputs))
        x = F.relu(self.fc(x))
        if is_training:
            x = self.dropout(x)
        return x, self.sigmoid(self.classifier(x))
    
    def forward_camC(self, inputs, is_training=True):
        x = F.relu(self.fc0_camC(inputs))
        x = F.relu(self.fc(x))
        if is_training:
            x = self.dropout(x)
        return x, self.sigmoid(self.classifier(x))

    def forward_camD(self, inputs, is_training=True):
        x = F.relu(self.fc0_camD(inputs))
        x = F.relu(self.fc(x))
        if is_training:
            x = self.dropout(x)
        return x, self.sigmoid(self.classifier(x))

    def forward_camE(self, inputs, is_training=True):
        x = F.relu(self.fc0_camE(inputs))
        x = F.relu(self.fc(x))
        if is_training:
            x = self.dropout(x)
        return x, self.sigmoid(self.classifier(x))
...
class Model_single_Mtv2(torch.nn.Module):
    """
    @summary: multiview network - Date 22 Nov 2023
    """
    def __init__(self, n_feature):
        super(Model_single_Mtv2, self).__init__()

        self.fc_camA = nn.Linear(n_feature, n_feature)
        self.fc_camB = nn.Linear(n_feature, n_feature)
        self.fc_camC = nn.Linear(n_feature, n_feature)
    ...
    def forward_camA(self, inputs, is_training=True):
        x = F.relu(self.fc_camA(inputs))
        if is_training:
            x = self.dropout(x)
        return x, self.sigmoid(self.classifier(x))
    
    def forward_camB(self, inputs, is_training=True):
        x = F.relu(self.fc_camB(inputs))
        if is_training:
            x = self.dropout(x)
        return x, self.sigmoid(self.classifier(x))
    
    def forward_camC(self, inputs, is_training=True):
        x = F.relu(self.fc_camC(inputs))
        if is_training:
            x = self.dropout(x)
        return x, self.sigmoid(self.classifier(x))
    
    def forward_camD(self, inputs, is_training=True):
        x = F.relu(self.fc_camD(inputs))
        if is_training:
            x = self.dropout(x)
        return x, self.sigmoid(self.classifier(x))

    def forward_camE(self, inputs, is_training=True):
        x = F.relu(self.fc_camE(inputs))
        if is_training:
            x = self.dropout(x)
        return x, self.sigmoid(self.classifier(x))
...
def model_generater(model_name, feature_size):
    ...
    elif model_name == 'model_single_mtv':
        model = Model_single_Mtv(feature_size)  # for anomaly detection, only one class, anomaly, is needed.
    elif model_name == 'model_single_mtv2':
        model = Model_single_Mtv2(feature_size)  # for anomaly detection, only one class, anomaly, is needed.
