...
def train(epochs, train_loader, test_loader, args, model, optimizer, logger, device, save_path, 
    ...
    for epoch in range(1, epochs+1):
        ...
        for i, data in enumerate(train_loader):
            itr += 1
            [anomaly_features_camA, normaly_features_camA],[anomaly_features_camB, normaly_features_camB],[anomaly_features_camC, normaly_features_camC],[anomaly_features_camD, normaly_features_camD],[anomaly_features_camE, normaly_features_camE], [anomaly_label, normaly_label], stastics_data = data
            ...
            if args.loss_combination == "PairwiseMeanOfClipScoresInBags":
                ...
                # camD
                features_camD = torch.cat((anomaly_features_camD.squeeze(0), normaly_features_camD.squeeze(0)), dim=0)
                videolabels_camD = torch.cat((anomaly_label.squeeze(0), normaly_label.squeeze(0)), dim=0)
                seq_len_camD = torch.sum(torch.max(features_camD.abs(), dim=2)[0] > 0, dim=1).numpy()
                features_camD = features_camD[:, :np.max(seq_len_camD), :]
                features_camD = features_camD.float().to(device)
                videolabels_camD = videolabels_camD.float().to(device)
                if args.model_name == 'model_single':
                    final_features_camD, element_logits_camD = model(features_camD)
                else:
                    # *.forward_cam2
                    final_features_camD, element_logits_camD = model.forward_camD(features_camD)


                # camE
                features_camE = torch.cat((anomaly_features_camE.squeeze(0), normaly_features_camE.squeeze(0)), dim=0)
                videolabels_camE = torch.cat((anomaly_label.squeeze(0), normaly_label.squeeze(0)), dim=0)
                seq_len_camE = torch.sum(torch.max(features_camE.abs(), dim=2)[0] > 0, dim=1).numpy()
                features_camE = features_camE[:, :np.max(seq_len_camE), :]
                features_camE = features_camE.float().to(device)
                videolabels_camE = videolabels_camE.float().to(device)
                if args.model_name == 'model_single':
                    final_features_camE, element_logits_camE = model(features_camE)
                else:
                    # *.forward_cam2
                    final_features_camE, element_logits_camE = model.forward_camD(features_camE)



                weights = args.Lambda.split('_')

                element_logits = (element_logits_camA+element_logits_camB+element_logits_camC+element_logits_camD+element_logits_camE)/5
            ...
            weights = args.Lambda.split('_')
            m_loss_camD = KMXMILL_individual(element_logits=element_logits_camD, seq_len=seq_len_camD, labels=videolabels_camD, device=device, loss_type='CE', args=args)
            n_loss_camD = normal_smooth(element_logits=element_logits_camD, labels=videolabels_camD, device=device)
            total_loss_camD = float(weights[0]) * m_loss_camD + float(weights[1]) * n_loss_camD
            logger.log_value('m_loss', m_loss_camD, itr)
            logger.log_value('n_loss', n_loss_camD, itr)
            ...
            weights = args.Lambda.split('_')
            m_loss_camE = KMXMILL_individual(element_logits=element_logits_camE, seq_len=seq_len_camE, labels=videolabels_camE, device=device, loss_type='CE', args=args)
            n_loss_camE = normal_smooth(element_logits=element_logits_camE, labels=videolabels_camE, device=device)
            total_loss_camE = float(weights[0]) * m_loss_camE + float(weights[1]) * n_loss_camE
            logger.log_value('m_loss', m_loss_camE, itr)
            logger.log_value('n_loss', n_loss_camE, itr)



            # max
            if args.loss_combination == "Max":
                total_loss = torch.max(total_loss_camA, 
                                       torch.max(total_loss_camB, 
                                                 torch.max(total_loss_camC, 
                                                           torch.max(total_loss_camD, total_loss_camE)
                                                ) 
                                        )
                            )
        ...
        if epoch % args.save_every == 0:
            ...
            results = eval_p(epoch = epoch, itr=itr, 
                             ...
                             dataset_camD = args.dataset_name_camD, 
                             dataset_camE = args.dataset_name_camE, 
