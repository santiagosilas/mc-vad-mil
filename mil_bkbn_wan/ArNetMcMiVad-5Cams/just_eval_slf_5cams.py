...
from test import test_slf_5cams
...
if __name__ == '__main__':
    ...
    model4 = model_generater(model_name=args.model_name, feature_size=args.feature_size).to(device)
    model5 = model_generater(model_name=args.model_name, feature_size=args.feature_size).to(device)
    
    ckpt1 = torch.load(args.pretrained_ckpt1)
    ckpt2 = torch.load(args.pretrained_ckpt2)
    ckpt3 = torch.load(args.pretrained_ckpt3)
    ckpt4 = torch.load(args.pretrained_ckpt4)
    ckpt5 = torch.load(args.pretrained_ckpt5)

    model1.load_state_dict(ckpt1)
    model2.load_state_dict(ckpt2)
    model3.load_state_dict(ckpt3)
    model4.load_state_dict(ckpt4)
    ...
    test_result_dict = test_slf_5cams(test_loader, model1, model2, model3, model4, model5, device, args)
