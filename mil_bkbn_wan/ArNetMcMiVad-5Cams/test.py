...
def test(test_loader, model, device, args):
    ...
    for i, data in enumerate(test_loader):
        ...
        feature_camA, feature_camB, feature_camC, feature_camD, feature_camE, data_video_name = data
        ...
        feature_camE = feature_camE.to(device) 
        ...
        with torch.no_grad():
            ...
            if args.model_name == 'model_single':
                _, element_logits_camE = model(feature_camE.float(), is_training=False)
            else:
                # *.forward_cam2
                _, element_logits_camE = model.forward_camD(feature_camE.float(), is_training=False)
        ...
        element_logits_camE = element_logits_camE.cpu().data.numpy().reshape(-1)
        ...
        result[data_video_name[0]] = {
            ...
            "camE":element_logits_camE,
        }
    
    return result


def test_slf_5cams(test_loader, model1, model2,model3,model4, model5, device, args):
    result = {}
    for i, data in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            # camera E
            if args.model_name == 'model_single':
                _, element_logits_camE = model4(feature_camE.float(), is_training=False)
            else:
                # *.forward_cam2
                _, element_logits_camE = model4.forward_camE(feature_camE.float(), is_training=False)
