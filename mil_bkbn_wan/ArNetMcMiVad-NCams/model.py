...
class Model_single_Mtv2(torch.nn.Module):
    def __init__(self, n_feature, view_names):
        super(Model_single_Mtv2, self).__init__()

        #self.fc_cams = {view_name:nn.Linear(n_feature, n_feature).to("cuda:0") for view_name in view_names}
        self.fc_cams = nn.ModuleDict()
        for view_name in view_names:
            self.fc_cams[view_name] = nn.Linear(n_feature, n_feature).to("cuda:0")
    ...
    def forward_with_cam(self, view_name, inputs, is_training=True):
        x = F.relu(self.fc_cams[view_name](inputs))
        if is_training:
            x = self.dropout(x)
        return x, self.sigmoid(self.classifier(x))
...
def model_generater(model_name, feature_size, camera_list):
    ...
    if model_name == 'model_single':
        ...
        model = Model_single_Mtv2(feature_size, camera_list)  # for anomaly detection, only one class, anomaly, is needed.
