...
def main(parser):
    ...
    camera_list_key = "-".join(args.camera_list)
    save_path = os.path.join(args.model_name, args.feature_pretrain_model, f"{camera_list_key}", 
    ...
    model = model_generater(model_name=args.model_name, feature_size=args.feature_size, camera_list = args.camera_list).to(device)
    ...
    ckpt_path = os.path.join(os.path.join(os.path.join(args.dataset_path, f"outputs-{camera_list_key}"), "ckpt"), save_path)
    if not os.path.exists(ckpt_path):
        os.makedirs(ckpt_path)
    print(ckpt_path)
    
    logs_path = os.path.join(os.path.join(os.path.join(args.dataset_path, f"outputs-{camera_list_key}"), "logs"), save_path)
    ...
    results_path = os.path.join(os.path.join(os.path.join(args.dataset_path, f"outputs-{camera_list_key}"), "result"), save_path)
    print(results_path)
